...
const createRecipe = (req, res) => {
    //! Este userId hace referencia al usuario logeado
    // ?si esta ruta es protegida siempre va a existir userId
    const userId = req.user.id

    const {title, description, urlImg, time, portions, categoryId, origin} = req.body
    // * en los servicios los if deben contener else    
...
const patchRecipe = (req, res) => {
    ...
    recipeControllers.updateRecipe(id, {title, description, urlImg, time, portions, categoryId, origin})
        .then(data => {
            // !si updateRecipe modifica la receta data[0]=true 
            if(data[0]){
...
const getUserRecipes =(req,res)=>{
    ...
    .catch(err=>{
        res.status(400).jso({message:err.mesage})
    })
}
